...
def calculate_opt_nb_clusters(num_nodes):
    """ Calculate the optimal number of clusters for Modified Leach Algorithm """
    # An algorithm for finding the optimal number of clusters in the network
    # TODO Add Reference to Paper
    cf.NB_CLUSTERS = int(np.round(np.sqrt(0.5 * num_nodes * (cf.THRESHOLD_DIST/np.pi) *
                                          ((cf.AREA_LENGTH/2)/(0.765 * cf.CLUSTER_BASE_DIST**2)))))
    return cf.NB_CLUSTERS
...
def save_timer_logs2csv(timer_logs):
    to_csv = []
    dir_path = get_result_path()
    for scenario_name, timer_log in timer_logs.items():
        for timer in timer_log.values():
            tmp = {'node_id': timer.node_id,
                   'stage': timer.stage.name,
                   }
            to_csv.append(tmp)
        df = pd.DataFrame(to_csv)
        df.to_csv("{0}{2}_{1}-timer-log.csv".format(dir_path,
                                                    scenario_name, time.strftime("%H_%M_%S")))
...
def save2csv_raw(traces):
    ...
    for scenario_name, tracer in traces.items():
        ...
        df = pd.DataFrame(to_csv)
        df.to_csv("{0}{2}_{1}-cov_vs_sleeping.csv".format(dir_path,
                                                          scenario_name, time.strftime("%H_%M_%S")))
...
def save2csv(traces):
    ...
    df = pd.DataFrame(to_csv)
    dir_path = get_result_path()
    df.to_csv("{0}-{1}-results_summary.csv".format(dir_path,
...
def plot_clusters(network, save=False, round_nb=0, filename=None):
    colors = ['b', 'k', 'y', 'g', 'm', 'c']
    # print clusters
    plt.figure()
    for cluster_id in range(0, cf.NB_CLUSTERS):
        ...
        #[node for node in cluster if node.id !=0 ]
        #cluster = [node for node in cluster if node.id !=1 ]  
        X = [node.pos_x for node in cluster if not node.is_head()]
        Y = [node.pos_y for node in cluster if not node.is_head()]
    ...
    x = network.get_alive_nodes()

#     for cluster_id in range(0, nb_clusters):
#         X = [node.pos_x for node in network.get_alive_nodes()]
#         Y = [node.pos_y for node in network.get_alive_nodes()]
#         Z = [1 if node.membership == cluster_id else 0 for node in network.get_alive_nodes()]
#         X, Y, Z = grid(X, Y, Z)
#         plt.contour([X, Y], Z, colors='0.6')
    ...
    if save:
        plt.savefig(filename if filename else f"round-{round_nb}-network-structure.png")
        plt.close()
    else:
        plt.show()
...
def plot_time_of_death(network):
    ...
    x = [node.pos_x for node in network.get_alive_nodes()]
    y = [node.pos_y for node in network.get_alive_nodes()]
    z = [node.time_of_death for node in network.get_alive_nodes()]
...
def log_curves(curves):
    ...
    df.to_csv("{0}-{1}-alive_nodes.txt".format(dir_path,
                                               time.strftime("%H_%M_%S")))
...
def log_coverages(pso_wrapper):
    ...
    df.to_csv("{0}-{1}-cov_log.txt".format(dir_path,
                                           time.strftime("%H_%M_%S")))
